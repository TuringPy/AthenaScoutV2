...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $query = "UPDATE abonnement_base SET stock = stock - :quantite WHERE id = 3"; // Remplacer "1" par l'ID de l'abonnement souhaité
  $stmt = $pdo->prepare($query);
  $stmt->bindValue(':quantite', $quantite, PDO::PARAM_INT);
  $stmt->execute();
  
  // Calculer le montant total de l'achat
  $query = "SELECT prix FROM abonnement_base WHERE id = 3"; // Remplacer "1" par l'ID de l'abonnement souhaité
  $stmt = $pdo->query($query);
  $abonnement = $stmt->fetch(PDO::FETCH_ASSOC);
